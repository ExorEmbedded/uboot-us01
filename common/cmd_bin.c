 /* This file allows booting nk.bin WCE6 images from RAM or NOR
  *
...
int valid_bin_image (unsigned long addr)
{
	...
	ulong dest = CONFIG_SYS_LOAD_ADDR;
	/* If we are reading from AMBA FLASH ... we need to support paging 
	 * read just a little piece of image for getting the header 
	 */
	if (addr2info(addr) != NULL)
	{
	  flash_read((ulong) addr, (ulong *)dest, (ulong)(100));
	  sync = (struct bin_sync*) dest;
	}
	else
#endif
...
#if defined(CONFIG_AMBA_FLASH)
	ulong dest = CONFIG_SYS_LOAD_ADDR;
	/* If we are reading from AMBA FLASH ... we get the header from 
	 * the RAM buffer area starting at CONFIG_SYS_LOAD_ADDR
	 */
	if (addr2info(addr) != NULL)
	{
	  memcpy((char*)&header, (char*)(dest + NUM_SYNCBYTES), sizeof(header));
	}
	else
#endif
...
#if defined(CONFIG_AMBA_FLASH)
	dest = CONFIG_SYS_LOAD_ADDR;
	/* If we are reading from AMBA FLASH ... now we know the length 
	 * so we can copy the full image to RAM buffer and point to it
	 */
	if (addr2info(addr) != NULL)
	{
  	  flash_read((ulong) addr, (ulong *)dest, (ulong)((header.img_length)/4+100));
	  current_addr = dest + sizeof(struct bin_header) + NUM_SYNCBYTES;
	}
	else
#endif

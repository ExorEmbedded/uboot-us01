...
int  env_init(void)
{
#ifdef CONFIG_AMBA_FLASH	/* It is necessary to copy FLASH into buffer to prevent misaligned data read of crc32 routine */
	ulong crc, new;
	uchar* bufpnt = (uchar*)CONFIG_SYS_LOAD_ADDR;
	
	/* read old CRC */
	flash_read((ulong)&(env_ptr->crc), &crc, 1);
	/* crc = env_ptr->crc; */
	/* copy environment into temp buffer */
	flash_read((ulong)&(env_ptr->data), (ulong *)bufpnt, ENV_SIZE/4);

	/* Calculate new crc and check */
	new = crc32(0, bufpnt, ENV_SIZE);
	if (crc == new) 
	{
...
#endif /* CONFIG_ENV_ADDR_REDUND */
#ifdef CMD_SAVEENV
#ifdef CONFIG_AMBA_FLASH
	flash_read((ulong) flash_addr, (ulong *)env_ptr, CONFIG_ENV_SIZE/4); 
#else 

...
ulong genimg_get_image (ulong img_addr)
{
	...
	if (addr2info(img_addr) != NULL){
		...
		flash_read((ulong) img_addr, (ulong *)ram_addr, h_size/4);
		/* get data size */
		switch (genimg_get_format ((void *)ram_addr)) {
		case IMAGE_FORMAT_LEGACY:
			d_size = image_get_data_size ((image_header_t *)ram_addr);
		...
		flash_read((ulong) (img_addr+h_size), (ulong *)(ram_addr+ h_size), d_size/4+1);
	}
#endif /* CONFIG_AMBA_FLASH */

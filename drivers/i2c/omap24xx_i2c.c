 ...
 *
 * Modified by: Giovanni Pavoni Sitek S.p.a. 04/11/2010
 * Reason: Added subaddress mode 0 write support, in order to interface with TSC2004 I2C touch controller
 */
 ...
 */
static int i2c_write_sa0 (u8 devaddr, uchar * buffer, int len)
{
	int i2c_error = 0;
	u16 status, stat;
	int i;
...
int i2c_write (uchar chip, uint addr, int alen, uchar * buffer, int len)
{
	...
	if(alen == 0)
	{
	  if( i2c_write_sa0 (chip, buffer, len))
	  {
			printf ("I2C write SA0: I/O error\n");
			i2c_init (CONFIG_SYS_I2C_SPEED, CONFIG_SYS_I2C_SLAVE);
			return 1;
	  }
	  else

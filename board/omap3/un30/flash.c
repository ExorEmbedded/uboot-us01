...
#define FLASH_BANK_SIZE (FLASH_BANK_RANGE + 1)
...
#ifdef CONFIG_SYS_FLASH_DMA
			/* Use DMA copy for reading one whole 4MB bank */
			if( (((addr+4*i)&FLASH_BANK_RANGE)==0) &&  ((len - i) >= (FLASH_BANK_SIZE >> 2)))
			{
//		  	  printf ("DMA+ \n"); //!!!
			  /* DMA init sequence */
			  dma4_write_reg(DMA4_OCP_SYSCONFIG, 0x00001329);

			  dma4_write_reg(DMA4_CSDP0, 0x00000002);	//Little endian, single access, 32 bit data transfer, non posted
			  dma4_write_reg(DMA4_CEN0,  256);			//Number of element per frame (256 long = 1KB)
			  dma4_write_reg(DMA4_CFN0,  (FLASH_BANK_SIZE/1024));		//Number of frames per block

			  dma4_write_reg(DMA4_CSSA0, PHYS_FLASH_1);	//Source start address
			  reg = (ulong)(buf + i);
			  ...
			  i += (FLASH_BANK_SIZE >> 2) - 1; 			// Point to the last word of the copied bank
//		  	  printf ("DMA-\n"); //!!!
			}
#endif
		}

 ...
 * MA 02111-1307 USA
 *
 * NOTE: Modified version for un31p platform (no FPGA/Amba bus; linear addressing used)
 * Modified by: Giovanni Pavoni Sitek S.p.a.
 */

#include <common.h>
#include <linux/byteorder/swab.h>
#include <asm/io.h>
#include <asm/arch/dma4.h>
#include <touch.h>
 ...
 * NOTE: S29GL01GP on GPMC version (function is empty because of linear addressing)
 */
void flash_setpage(ulong addr)
{
}
 ...
 * NOTE: S29GL01GP on GPMC version (linear addressing)
 */
void flash_read(ulong addr, ulong *buf, ulong len)
{
	ulong i;
	static ushort* pnt; 
#ifdef CONFIG_SYS_FLASH_DMA 
	ulong framesize;		//Size of each frame (1KB) (DMA)
	ulong nframes;			//Number of frames for the transfer block (DMA)
	ulong reg;
#endif	
	pnt = (ushort*)buf;
...
#ifdef CONFIG_SYS_FLASH_DMA
	/* Use DMA copy for reading blocks of 1KB or more */
	if(len >= 256)
	{
	  framesize = 512;					//512 16-bit datas = 1KB
	  nframes = (len * 2) / framesize;	//number of frames to be copied
  
	  /* DMA init sequence */
  
	  dma4_write_reg(DMA4_OCP_SYSCONFIG, 0x00001329);
	  dma4_write_reg(DMA4_CSDP0, 0x00000001);	//Little endian, single access, 16 bit data transfer, non posted
	  dma4_write_reg(DMA4_CEN0,  framesize);	//Number of element per frame
	  dma4_write_reg(DMA4_CFN0,  nframes);		//Number of frames per block
	  dma4_write_reg(DMA4_CSSA0, addr);			//Source start address
	  reg = (ulong)buf;
	  dma4_write_reg(DMA4_CDSA0, reg);			//Destination start address
	  dma4_write_reg(DMA4_CCR0,  0x00005000);	//Read and write with post increment
	  dma4_write_reg(DMA4_CSEl0, 0x00000001);	//Source element 1
	  dma4_write_reg(DMA4_CSFl0, 0x00000001);	//Source frame 1
	  dma4_write_reg(DMA4_CDEl0, 0x00000001);	//Dest element 1
	  dma4_write_reg(DMA4_CDFl0, 0x00000001);	//Dest frame 1
  
	  reg = dma4_read_reg(DMA4_CCR0);			//Start DMA
	  reg |= 0x80;
	  dma4_write_reg(DMA4_CCR0, reg);
	  
	  /* Wait for DMA complete */
  	  reg = dma4_read_reg(DMA4_CCR0); 			
	  while(reg & 0x80)
	  { 
		reg = dma4_read_reg(DMA4_CCR0);
#ifdef CONFIG_TOUCH	
		// If moving a very big block ... update touch transitons counter during polling loop for completion
		if(len > 0x100000)
		  touch_get_tap_tap();
#endif
	  }
	  
	  /* Manually copy the remainder */
	  for(i=framesize*nframes; i<(2*len); i++)
	  {
		  pnt[i]=ReadAddr16(addr + 2*i); // read data
	  }
	  return;
	}
...
static void WriteBufferBurst(ulong addr, uchar *dat, uchar len)
  {
	...
	WriteAddr16(PHYS_FLASH_1 + eff_addr, 0x0025);

	// Write data length - 1
	WriteAddr16(PHYS_FLASH_1 + eff_addr, eff_len-1);
	// Write data
    ...
    for(i=0; i<eff_len; i++)
	{
	  tmp_data = *tmppnt;
      tmppnt++;
	  WriteAddr16(PHYS_FLASH_1 + (addr+(i*2)), tmp_data);
	}

	//confirm write
	WriteAddr16(PHYS_FLASH_1 + eff_addr, 0x0029);
...
static char WaitUntilFinished(ulong addr, uchar expected_data, int type)
{
	...
	while(1){
		read_data1 = ReadAddr16(PHYS_FLASH_1 + eff_addr);
		read_data1 &= 0xFFFF;
		if(!((read_data1 ^ eff_data) & 0x0080)){ // polling finished
			read_data2 = ReadAddr16(PHYS_FLASH_1 + eff_addr);
			read_data3 = ReadAddr16(PHYS_FLASH_1 + eff_addr);
			read_data2 &= 0xFFFF;
			read_data3 &= 0xFFFF;
...
static void sector_erase(ulong secaddr)
{
	...
	WriteAddr16(PHYS_FLASH_1 + secaddr, 0x0030);
	// wait until operation is finished
